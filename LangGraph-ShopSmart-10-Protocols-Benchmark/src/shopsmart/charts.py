...
def plot_comparison(langsmith_results: list[dict], langfuse_results: list[dict]) -> None:
    ...
    plt.tight_layout()
    out_path = get_output_dir() / "observability_report.png"
...
def plot_routing_distribution(category_counts: dict) -> None:
    ...
    plt.tight_layout()
    out_path = get_output_dir() / "routing_distribution.png"
    plt.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[Charts] Saved → {out_path}")
...
def plot_tool_usage(tool_usage: dict) -> None:
    ...
    plt.tight_layout()
    out_path = get_output_dir() / "tool_usage.png"
    plt.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[Charts] Saved → {out_path}")


def plot_protocol_comparison(protocol_stats: dict) -> None:
    """Plot p50/p95 latency per protocol as a grouped bar chart."""
    try:
        import matplotlib.pyplot as plt
    except ImportError:
        print("[Charts] matplotlib not installed — skipping plots.")
    ...
    if not protocol_stats:
        print("[Charts] No protocol results to plot.")
        return

    protocols = sorted(protocol_stats.keys())
    p50 = [protocol_stats[p]["p50_latency_ms"] for p in protocols]
    p95 = [protocol_stats[p]["p95_latency_ms"] for p in protocols]

    x = range(len(protocols))
    width = 0.35
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar([i - width / 2 for i in x], p50, width, label="p50", color="#0F62FE")
    ax.bar([i + width / 2 for i in x], p95, width, label="p95", color="#FF6F00")
    ax.set_xticks(list(x))
    ax.set_xticklabels(protocols)
    ax.set_ylabel("Latency (ms)")
    ax.set_title("Protocol Latency Comparison (p50 / p95)", fontweight="bold")
    ax.legend()
    ax.grid(axis="y", alpha=0.3)
    plt.tight_layout()
    out_path = get_output_dir() / "protocol_comparison.png"
    plt.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[Charts] Saved → {out_path}")
